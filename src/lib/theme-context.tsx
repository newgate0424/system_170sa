...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  // Apply theme to document (run only once after loading)
  useEffect(() => {
    if (isLoading) return
  ...
  }, [colors.primary, colors.background, isLoading])

  // Apply fonts (separate effect)
  useEffect(() => {
    if (isLoading) return
  ...
  }, [fonts.family, fonts.size, isLoading])
