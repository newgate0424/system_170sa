...
export async function GET(req: NextRequest) {
  try {
    ...
    // Group by adser (not team)
    const adserMap = new Map<string, any[]>();
    filteredData.forEach((record: any) => {
      const adser = record.adser || 'Unknown';
      if (!adserMap.has(adser)) {
        adserMap.set(adser, []);
      }
      adserMap.get(adser)!.push(record);
    });

    // Transform to TeamMetric[]
    const teamMetrics: TeamMetric[] = [];

    adserMap.forEach((records, adserName) => {
      // Aggregate totals
      ...
      records.forEach((record: any) => {
        ...
        // For adser, use newPlayerRevenueAdser instead of newPlayerRevenueTeam
      ...
      teamMetrics.push({
        team_name: adserName, // Use adser name as team_name
